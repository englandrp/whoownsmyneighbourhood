...
function initHome(postcode, easting, northing)
{
	...
	$('#postcodeform').submit(function(e) {
		...
		{
			var substr = ward.split('/');
			var point = new OpenSpace.MapPoint(substr[0], substr[1], 2);
			showPostcodeResult(point);
		}
	});
...
{
	if (mapPoint.lat > 397000 && mapPoint.lat < 435000 && mapPoint.lon > 394000 && mapPoint.lon < 434000)
	{
	...
	{
		alert('Sorry - that postcode is out of the range covered by this service.');
	}
}
...
function openPopup(feature)
{
	...
	if (feature.attributes.descr != '')
	{
		...
		popupContent += "<br /><a href=\"" + feature.attributes.url + "\">View plot details</a>";
	}
	else
	{
		//popupContent += "<a href=\"/plot/" + feature.attributes.plot_id + "\">View plot details</a>";
		popupContent += "<a href=\"" + feature.attributes.url + "\">View plot details</a>";
